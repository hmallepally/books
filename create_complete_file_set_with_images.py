...
def copy_images():
    ...
    # Copy any other images in root
    for image_file in os.listdir("."):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')) and image_file != "book_title_image.png":
            shutil.copy2(image_file, f"final_versions/with_toc/images/{image_file}")
            shutil.copy2(image_file, f"final_versions/without_toc/images/{image_file}")
            shutil.copy2(image_file, f"final_versions/individual_distribution/images/{image_file}")
            print(f"  📸 Copied: {image_file}")
