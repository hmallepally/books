#!/usr/bin/env python3
"""
Fix content properly by working with actual HTML structure
"""

def fix_content_properly():
    """Fix content by working with actual HTML structure"""
    import re
    
    print("🚀 Fixing content properly by working with actual HTML structure...")
    ...
    # Find Chapter 1 overview section and add story after it
    chapter1_overview_pattern = r'(<h3 id="chapter-overview">Chapter Overview</h3>.*?</div>\s*)(<div class="page-break"></div>)'
    sarah_story = '''
    ...
    # Find Chapter 2 overview section and add story after it
    chapter2_overview_pattern = r'(<h3 id="chapter-overview-1">Chapter Overview</h3>.*?</div>\s*)(<div class="page-break"></div>)'
    marcus_story = '''
    <div class="story-section">
    ...
    html_content = re.sub(
        chapter2_overview_pattern,
        r'\1' + marcus_story + r'\2',
        html_content,
        flags=re.DOTALL
    )
    ...
    output_file = 'Operational_Excellence_with_AI_CONTENT_PROPERLY_FIXED.html'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ Content properly fixed HTML created: {output_file}")
    return output_file

def generate_content_properly_fixed_pdf(html_file):
    """Generate PDF from content properly fixed HTML"""
    import asyncio
    ...
    async def generate_pdf():
        async with async_playwright() as p:
            ...
            output_pdf = 'Operational_Excellence_with_AI_CONTENT_PROPERLY_FIXED.pdf'
            await page.pdf(
                path=output_pdf,
                format='A4',
                margin={
            ...
            await browser.close()
            print(f"🎉 Content Properly Fixed PDF generated: {output_pdf}")
...
if __name__ == "__main__":
    ...
    # Fix content properly
    html_file = fix_content_properly()
    
    if html_file:
        # Generate content properly fixed PDF
        print("\n📚 Generating content properly fixed PDF...")
        generate_content_properly_fixed_pdf(html_file)
        
        print("\n✅ CONTENT PROPERLY FIXED!")
        ...
        print("❌ Failed to fix content properly")
