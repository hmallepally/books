#!/usr/bin/env python3
"""
Manual fix - find exact locations and insert content manually
"""

def manual_fix():
    """Manually fix content by finding exact locations"""
    
    print("🚀 Manual fix - finding exact locations and inserting content...")
    ...
    # Add Chapter 16 to TOC manually
    ...
    # Find the exact location in TOC
    toc_location = html_content.find('<li class="chapter"><a href="#chapter15">Chapter 15: AI Tools and Technologies: A Practical Guide <span class="toc-page-number">285</span></a></li>')
    if toc_location != -1:
        # Find the end of this line
        end_location = html_content.find('</li>', toc_location) + 5
        # Insert Chapter 16 after this line
        html_content = html_content[:end_location] + '\n' + chapter16_toc + html_content[end_location:]
        print("✅ Chapter 16 added to TOC")
    else:
        print("❌ TOC location not found")
    
    print("📝 2. Adding Sarah Chen story to Chapter 1...")
    
    # Find Chapter 1 overview and add story after it
    chapter1_overview_location = html_content.find('<h3 id="chapter-overview">Chapter Overview</h3>')
    if chapter1_overview_location != -1:
        # Find the end of the overview paragraph
        overview_end = html_content.find('</p>', chapter1_overview_location)
        overview_end = html_content.find('</p>', overview_end + 4)  # Second </p>
    ...
    '''
        ...
        html_content = html_content[:overview_end + 4] + sarah_story + html_content[overview_end + 4:]
        print("✅ Sarah Chen story added to Chapter 1")
    else:
        print("❌ Chapter 1 overview not found")
    
    print("📝 3. Adding Marcus Rodriguez story to Chapter 2...")
    
    # Find Chapter 2 overview and add story after it
    chapter2_overview_location = html_content.find('<h3 id="chapter-overview-1">Chapter Overview</h3>')
    if chapter2_overview_location != -1:
        # Find the end of the overview paragraph
        overview_end = html_content.find('</p>', chapter2_overview_location)
        overview_end = html_content.find('</p>', overview_end + 4)  # Second </p>
    ...
    '''
        ...
        html_content = html_content[:overview_end + 4] + marcus_story + html_content[overview_end + 4:]
        print("✅ Marcus Rodriguez story added to Chapter 2")
    else:
        print("❌ Chapter 2 overview not found")
    
    print("📝 4. Adding self-assessment to Chapter 1...")
    
    # Find Chapter 1 conclusion and add assessment before it
    ...
    '''
        ...
        html_content = html_content[:chapter1_conclusion_location] + assessment + html_content[chapter1_conclusion_location:]
        print("✅ Self-assessment added to Chapter 1")
    else:
    ...
    output_file = 'Operational_Excellence_with_AI_MANUAL_FIX.html'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ Manual fix HTML created: {output_file}")
    return output_file

def generate_manual_fix_pdf(html_file):
    """Generate PDF from manual fix HTML"""
    import asyncio
    ...
    async def generate_pdf():
        async with async_playwright() as p:
            ...
            output_pdf = 'Operational_Excellence_with_AI_MANUAL_FIX.pdf'
            await page.pdf(
                path=output_pdf,
                format='A4',
                margin={
            ...
            await browser.close()
            print(f"🎉 Manual Fix PDF generated: {output_pdf}")
...
if __name__ == "__main__":
    ...
    # Manual fix
    html_file = manual_fix()
    
    if html_file:
        # Generate manual fix PDF
        print("\n📚 Generating manual fix PDF...")
        generate_manual_fix_pdf(html_file)
        ...
        print("4. ✅ Self-assessment in Chapter 1 (before conclusion)")
