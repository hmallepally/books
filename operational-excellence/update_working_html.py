#!/usr/bin/env python3
"""
Update the working HTML file directly with new content enhancements
"""

def update_working_html_with_new_content():
    """Update the working HTML file with new content from markdown"""
    import re
    
    print("🚀 Updating working HTML file with new content enhancements...")
    
    # Read the working HTML file
    with open('Operational_Excellence_with_AI_COMPONENTS.html', 'r', encoding='utf-8') as f:
        html_content = f.read()
    
    print("📝 Adding new engaging stories to chapters...")
    
    # Add story to Chapter 1 (after the overview)
    chapter1_story = '''
    ...
    html_content = re.sub(
        r'(<h3>Chapter Overview</h3>.*?</div>\s*<div class="page-break"></div>)',
        r'\1' + chapter1_story,
        html_content,
        flags=re.DOTALL
    )
    ...
    html_content = re.sub(
        r'(<h3>Chapter Overview</h3>.*?<div class="page-break"></div>)',
        r'\1' + chapter2_story,
        html_content,
        flags=re.DOTALL
    )
    ...
    html_content = re.sub(
        r'(<h3>Common Pitfalls and How to Avoid Them</h3>.*?</div>\s*<div class="page-break"></div>)',
        r'\1' + chapter1_assessment,
        html_content,
        flags=re.DOTALL
    )
    ...
    output_file = 'Operational_Excellence_with_AI_ENHANCED_WORKING.html'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ Enhanced working HTML created: {output_file}")
    return output_file

def generate_pdf_from_enhanced_working_html(html_file):
    """Generate PDF from the enhanced working HTML"""
    import asyncio
    ...
    async def generate_pdf():
        async with async_playwright() as p:
            ...
            output_pdf = 'Operational_Excellence_with_AI_WORKING_ENHANCED.pdf'
            await page.pdf(
                path=output_pdf,
                format='A4',
                margin={
            ...
            await browser.close()
            print(f"🎉 Working Enhanced PDF generated: {output_pdf}")
...
if __name__ == "__main__":
    print("🚀 Updating working HTML file with new content...")
    
    # Update the working HTML file with new content
    html_file = update_working_html_with_new_content()
    
    if html_file:
        # Generate PDF from enhanced working HTML
        print("\n📚 Generating PDF from enhanced working HTML...")
        generate_pdf_from_enhanced_working_html(html_file)
        
        print("\n✅ SUCCESS!")
        print("📋 This PDF now includes:")
        print("1. ✅ All the working structure and formatting")
        print("2. ✅ New engaging stories (Chapters 1-2)")
        print("3. ✅ New self-assessment quiz (Chapter 1)")
        print("4. ✅ New 'Try This' exercise (Chapter 1)")
        print("5. ✅ Enhanced styling for new elements")
        print("\n📖 The PDF should now have the correct structure AND new content!")
    else:
        print("❌ Failed to update working HTML")
